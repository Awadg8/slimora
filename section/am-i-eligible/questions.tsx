      ...
      <div className="max-width">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-9">
          {data.map((item, index) => (
            <div
              key={index}
              className="z-20 flex flex-col justify-between rounded-[20px] bg-black/25 shadow-[0px_3.57px_3.57px_0px_#00000040] overflow-hidden"
            >
              ...
              <Link
                href={item.link}
                className="block text-center py-3 md:py-4 font-bold text-xs md:text-[14px] bg-[#2C5D6B]"
              >
                {item.linkText}
              </Link>
            </div>
