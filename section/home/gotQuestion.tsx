            ...
            <div className='max-width'>
                <div className='space-y-[19px] md:space-y-[17px]'>
                    ...
                    <p className='text-[12px] md:text-[20px] leading-[18px] md:leading-[24px] text-center md:text-left'>
                        From how SlimOra Gastric Balloon Systems works to clinical access, get the facts on the <br className='hidden md:block' /> world's first procedure less gastric balloon and digital care ecosystem.
                    </p>
                </div>

                <div className='pt-[23px] md:pt-[69px] md:space-y-[14px] space-y-1'>
                    {accordionData.map((item, index) => (
                        <div key={index} className="bg-[radial-gradient(circle,_#2C5D6B,_#0A1D20)] p-2 md:p-5">
                            <button
                                onClick={() => toggleAccordion(index)}
                                className="w-full flex justify-between items-center md:py-5 py-3 text-[#B5B5B5] cursor-pointer"
                            >
                                <span className="text-[#FFFFFF] text-[10px] md:text-[32px] font-semibold leading-[100%]">{item.question}</span>
                                <span className={`text-white transition-transform duration-1000 ease-in-out inline-block ${openIndex === index ? '-rotate-90' : ''}`}>
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="md:w-[30px] md:h-[55] w-3 h-6">
